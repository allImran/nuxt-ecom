<script setup lang="ts">
	...
	async function handlePrintPdf(order: InstantOrderListItem, event: Event) {
	  ...
	  try {
	    await generateInstantOrderPdf(order, props.business)
	  } catch (err) {
	    const toast = useToast()
	        ...
	        <thead class="bg-luxury-bg dark:bg-luxury-dark-bg border-b border-luxury-border dark:border-luxury-dark-border">
	          <tr>
	            <th
	              ...
	              :key="field.key"
	              :class="[
	                'px-6 py-3 text-left text-xs font-medium text-luxury-text-muted dark:text-luxury-dark-text-muted uppercase tracking-wider',
	                field.key !== 'actions' && 'cursor-pointer hover:text-luxury-gold transition-colors'
	              ]"
	              @click="field.key !== 'actions' && handleSort(field.key as SortableField)"
	            >
	        ...
	        <tbody class="divide-y divide-luxury-border dark:divide-luxury-dark-border">
	          ...
	          >
	            ...
	            <td class="px-6 py-4 whitespace-nowrap">
	              <button
	                type="button"
	                class="p-2 hover:bg-luxury-gold/10 rounded-lg transition-colors group"
	                :disabled="isGenerating"
	                :title="isGenerating ? 'Generating...' : 'Download PDF'"
	                @click="handlePrintPdf(order, $event)"
	              >
	                <UiIcon
	                  name="printer"
	                  :size="16"
