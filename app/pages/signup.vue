...
if (import.meta.server) {
  useSeoMeta({
    ...
    description: 'Create a new INDOORSHOPPING account to enjoy a personalized shopping experience.',
...
const handleSignup = async () => {
  const result = await authStore.register(identifier.value, password.value)
  if (result.success) {
    navigateTo('/')
  }
}
      ...
      <div class="bg-luxury-surface dark:bg-luxury-dark-surface p-8 md:p-12 rounded-luxury shadow-luxury dark:shadow-luxury-dark transition-colors duration-300">
        ...
        <div class="mt-8 text-center text-sm text-luxury-text-muted dark:text-luxury-dark-text-muted">
          <p>
            ...
            <NuxtLink to="/login" class="text-luxury-gold hover:text-luxury-gold-hover font-medium transition-colors">
              Sign in
            </NuxtLink>
          </p>
        </div>
